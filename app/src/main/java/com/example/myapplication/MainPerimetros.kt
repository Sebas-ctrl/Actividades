...
class MainPerimetros : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_main_perimetros)
        ...
        rb4.setOnClickListener {
            input1.isVisible = true
            input2.isVisible = false
            input1.hint = "Ingrese la lado del triángulo"
        ...
        btnCalculate.setOnClickListener {
            ...
            when ( radioSelected ){
                "cuadrado" -> {
                    total = (val1 * 4).toString()
                    result.text = total
                }
                "rectangulo" -> {
                    if (input2IsActivated) {
                        val val2 = input2.text.toString().toDouble()
                        total = (val1 * 2 + val2 * 2).toString()
                        result.text = total
                    }
                }
                "circulo" -> {
                    total = (2 * val1 * 3.1416).toString()
                    result.text = total
                }
                "triangulo" -> {
                    total = (val1 * 3).toString()
                    result.text = total
                }
            }
        }
